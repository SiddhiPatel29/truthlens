...
- **`VideoDetectionService`**:
  Streams video frames to a temporary disk file using a dynamic extension suffix (.mp4, .mov, .avi, .mkv), validates video duration limits (`MAX_VIDEO_DURATION_SECONDS = 120`) and resolution limits (`MAX_VIDEO_WIDTH = 4096`, `MAX_VIDEO_HEIGHT = 4096`, `MAX_VIDEO_PIXELS = 16_777_216`) on container metadata before frame processing, samples up to 16 keyframes uniformly, validates actual decoded frame dimensions upon reading each frame, computes frame-level Laplacian anomalies, calculates temporal instability, and renders a Grad-CAM++ style heatmap for the peak anomaly keyframe. In `finally`, deterministically executes `cap.release()` before `os.remove(temp_path)` to prevent Windows file handle lock leaks (`PermissionError [WinError 32]`).
